...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
  ...
  //cout<<"num_predicted="<<num_predicted<<"num_observations="<<num_observations<<endl;
  if (num_predicted==0)
    return;
  double min_dist;
  int landmark_index;
  for (int i=0;i<observations.size();i++) {
    ...
    min_dist=100000.0;//arbitrary large number
  ...
  // for (int i=0;i<observations.size();i++) {
  //   cout<<"Assigned observation "<<observations[i].id<<" to landmark"
  // }
}
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  // cout<<"Map size"<<map_landmarks.landmark_list.size();
  // cout<<"Obs size"<<observations.size();
  // cout<<"Sensor range"<<sensor_range;
  for (int i=0;i<num_particles;i++) {
